defmodule CustomerPanelWeb.EmailLive do
  ...
  def mount(_params, _session, socket) do
    ...
    # Sample rows akin to mockEmails
    rows = [
      %{
        received: "2025-08-24 08:41",
        from: "customer1@example.com",
        subject: "Quote request for deep clean",
        campaign: "Welcome",
        status: "Open",
        lastReply: "2025-08-24 09:10",
        assignee: "Alex"
      },
      %{
        received: "2025-08-24 11:05",
        from: "customer2@example.com",
        subject: "Scheduling follow-up",
        campaign: "Nurture",
        status: "Pending",
        lastReply: "2025-08-24 11:20",
        assignee: "Sam"
      },
      %{
        received: "2025-08-25 07:55",
        from: "biz@acme.co",
        subject: "PO and invoice",
        campaign: "Commercial",
        status: "Closed",
        lastReply: "2025-08-25 08:30",
        assignee: "Jamie"
      }
    ]
  ...
  def handle_event("row_click", %{"id" => id}, socket) do
    {:noreply, put_flash(socket, :info, "Email row clicked: #{id}")}
  end
